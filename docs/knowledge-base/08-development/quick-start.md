...
Три предыдущих режима работают на `localhost`, и этого достаточно для API и вёрстки. Но **Mini App внутри Telegram требует публичный HTTPS-адрес** — с `localhost` Telegram его не откроет. Бот при этом публичный адрес не требует: он ходит по polling и работает с локальной машины.

Одной командой:

```bash
npm run dev:miniapp -- --chat-id=<твой telegram id>
```

Команда поднимает туннель Cloudflare, записывает его адрес в `WEB_APP_URL`, обновляет кнопку меню в Telegram, собирает фронт и запускает сервер.
...
Скрипт ищет бинарь в `CLOUDFLARED_BIN`, затем в `~/.local/bin/cloudflared`, затем в `PATH`.

**Адрес одноразовый.** Бесплатный туннель выдаёт новый адрес при каждом запуске, а старый умирает. Отсюда два следствия, которые экономят полчаса разбирательств:

- После смены туннеля **отправь боту `/start`**. Кнопки в уже отправленных сообщениях хранят адрес внутри себя, и Telegram их не переписывает — старые кнопки будут вести на мёртвый туннель.
- Кнопку меню рядом с полем ввода скрипт обновляет через API, поэтому она всегда актуальна. Если сомневаешься, какая кнопка свежая, жми её.

Постоянный адрес требует именованного туннеля и домена в Cloudflare — см. FT-043 в `TASKS.md`.

---

### Ручной режим без Mini App

Когда телефон не нужен, короче так:

```bash
DATABASE_TYPE=sqlite ENABLE_TELEGRAM_POLLING=false npm run dev
```

Переопределение переменных в командной строке имеет приоритет над `.env.local`, поэтому файл править не нужно. Два флага здесь не случайны:

- `DATABASE_TYPE=sqlite` — в `.env.local` по умолчанию стоит `supabase`, а проект Supabase на паузе.
- `ENABLE_TELEGRAM_POLLING=false` — иначе процесс поднимет бота с боевым токеном, и он начнёт отвечать реальным людям.

### Тестовые данные

```bash
npm run seed:test -- --reset     # детерминированная фикстура
npm run smoke:semantics          # проверить, что итоги сходятся
npm run smoke:semantics -- --parse   # плюс разбор текста через OpenAI
```

Фикстура содержит по одной операции каждого семантического типа. Ожидаемые итоги — реальные расходы 2 532 000, доходы 12 000 000, две операции ждут решения — печатаются после засева и проверяются smoke-скриптом.
