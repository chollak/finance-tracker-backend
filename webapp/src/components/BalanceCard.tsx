  ...
  return (
    <Card className="relative overflow-hidden">
      <CardContent className="pt-6">
        {/* Trend Icon */}
        <div className="absolute top-6 right-6 w-10 h-10 rounded-full border-2 border-primary/30 flex items-center justify-center">
          <TrendingUp className="h-5 w-5" />
        </div>

        <p className="text-sm text-muted-foreground mb-3">My Balance</p>
        <p className={`${getFontSizeClass(balance)} font-bold mb-4 break-all overflow-hidden`}>
          ${balance.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
          <span className="text-muted-foreground">.{(balance % 1).toFixed(2).substring(2)}</span>
        </p>
        <p className="text-sm text-muted-foreground">
          Expenses this month have {isPositive ? 'increased' : 'decreased'}{' '}
          <span className="text-foreground font-semibold">{Math.abs(expenseChange)}%</span>
        </p>
      </CardContent>
    </Card>
  );
