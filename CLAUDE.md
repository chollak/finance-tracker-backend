...
**Current implementation queue (2026-09-04):** **пусто — безопасных активных задач локальной полировки не осталось.** Веха «Полировка под гайдлайны» закрыта целиком: `FT-059` (палитра диаграмм на 6 identity-цветов, health score в одной роли, статус долга без синего, гайдлайны под Onest), `FT-060` (русская плюрализация сведена в один хелпер `webapp/src/shared/lib/plural.ts`, счётчики на `/debts`, `/budgets`, `/transactions`, Home и в форме бюджета согласованы с числительным), `FT-061` (Onest self-hosted через `@fontsource-variable/onest` + `webapp/src/app/styles/fonts.css`, Google Fonts убран из `webapp/index.html`, `npm run check:fonts` в `npm run verify` стережёт возврат CDN). Шукур перенёс все оставшиеся не-quick-action задачи в backlog; текущий фокус — quick action dogfood и следующие friction-driven slices. Backlog включает `FT-049`/`FT-050` (GitHub-сверка, ветки), продовую auth-модель Shortcut из `FT-075`, применение backfill из `FT-045`, продуктовые решения (`FT-054`, `FT-062`, `FT-063`, `FT-069`, `FT-071`, `FT-051`), `FT-048` и `FT-046`. Не начинать backlog без явного запроса; новые задачи формулировать вокруг quick action и work one FT task at a time. Безопасный план quick capture закрыт полностью: гайд «как записывать операции» через Telegram (`FT-076`, `docs/IOS_SHORTCUT_ACTION_BUTTON.md`) и прямой Shortcut-вызов `POST /api/quick-capture` **для dev/test** (`FT-075`). Dev/test-контур: `X-Shortcut-User-Id` задаёт владельца и принудительно ставит `source=ios_shortcut`, опциональный `SHORTCUT_CAPTURE_TOKEN` проверяется через `X-Shortcut-Capture-Token` (плейсхолдер есть в `.env.example`), а при `NODE_ENV=production` весь обход отвергается с 403 `SHORTCUT_CAPTURE_DISABLED`. Это **не продовая auth-модель**: продовый токен (выдача, срок жизни, отзыв, отдельный rate limit для не-Telegram источника) — будущее решение Шукура, а не текущий блокер. Не включать обход в проде и не класть реальные токены в клиентские инструкции. `FT-049`/`FT-050` (GitHub-сверка, ветки) требуют явного разрешения на внешние действия. The live semantic smoke (`FT-044`), its debt-linked transaction bug (`FT-072`), read-only historical semantic preview (`FT-045`), budgets page UX cleanup (`FT-055`), transaction row readability (`FT-056`), collapsed semantic corrections (`FT-057`), touch target compliance (`FT-058`), the minimal Home/Capture slice (`FT-073`), the capture action row/states (`FT-074`), the iPhone Action Button guide (`FT-076`), the dev/test Shortcut capture API (`FT-075`), and the guideline polish trio (`FT-059`, `FT-060`, `FT-061`) are done/verified locally. Do not apply data backfills, close GitHub issues, delete branches, touch production/Supabase, or change product-policy without the explicit stop/ask rules below.
...
For current visual direction, read `docs/knowledge-base/10-design-guidelines/style-direction.md` before UI work. FT-059 reconciled the font (Onest 400–800; weight 500 allowed for captions/controls, not as a hierarchy step) and the chart palette; where any older guideline text still conflicts, prefer the active implementation + style-direction doc.
