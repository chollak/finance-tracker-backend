...
describe('И-4: порядок величины суммы не теряется при разборе', () => {
  ...
  // Починено в FT-067: parseAmount понимает множители, а незнакомый токен
  // рядом с числом заставляет отдать фразу модели вместо догадки.
  it('текст с множителем не теряет порядок величины', async () => {
