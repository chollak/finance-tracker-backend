...
| 8. Quick capture first | Главный экран = быстрый ввод, остальное ниже/в «Ещё» | FT-073 ✅, FT-074 ✅, FT-076 ✅ |
...
Пока blocked: не создавать новые публичные эндпоинты и не менять auth. Безопасный шаг без решения — документационный гайд «как записывать операции» (текст в Mini App, голос/текст в боте, Shortcut через отправку сообщения боту). Этот шаг выполнен в `FT-076`; сам прямой API остаётся blocked.
...
- [x] Прямой `POST /api/quick-capture` из Shortcut явно помечен как заблокированный до решения по auth/токену/отзыву/rate limit
