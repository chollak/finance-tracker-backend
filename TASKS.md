...
| 1.5. Доверие к цифрам | UI не противоречит семантике — **закрыта 2026-08-14**, кроме FT-068 | FT-052, FT-053, FT-054, FT-068 |
| 2. Качество ежедневного экрана | Список и бюджеты читаются и не врут — **закрыта 2026-08-14** | FT-055, FT-056, FT-057, FT-058 |
...
Предлагаемый подход (не утверждён): перевернуть проверку сложности с вычитания на сложение. Сейчас парсер спрашивает «выглядит ли это сложным» и по умолчанию считает незнакомое слово безобидным — отсюда и потерянный «млн», и «банкомат» из FT-068. Если он будет срабатывать только тогда, когда объяснил всю строку целиком, оба случая закрываются как класс, а не поштучно.

Альтернатива — выкинуть быстрый путь и всё отдавать модели — упирается в лимит 10 AI-запросов на 15 минут (FT-071) и задержку 3–8 секунд против 10 мс.

Definition of Done:
- [x] Быстрый парсер понимает множители
...
- [x] Тесты на все перечисленные формы
- [x] «1000000» без слова по-прежнему разбирается верно

Реализовано 2026-08-14 (TDD). Подход — переворот проверки, как и предлагалось.

Уточнение по ходу: буквальное «объяснить каждое слово» не работает — в «кофе 25000» слово «кофе» и есть описание, объяснять его нечем. Настоящий класс ошибки уже: **быстрый путь отбрасывал токен, примыкающий к числу и меняющий его смысл**. Поэтому правило: парсер не имеет права молча отбросить слово сразу после суммы — либо понимает, либо отдаёт фразу модели.

Разбор суммы вынесен в `src/modules/voiceProcessing/application/parseAmount.ts`, оба быстрых пути ходят через него. Заодно исчезли `SIMPLE_TRANSACTION_PATTERN` и `CURRENCY_WORDS_PATTERN` — их работу взял на себя один разборщик вместо двух похожих регулярок.

Вторая поправка на ходу: первая версия требовала, чтобы после числа стояло только слово валюты, и сломала фразы с числом в середине — «перевел 500000 на Alif». Предлоги образуют закрытый класс, в отличие от открытого множества существительных, поэтому они перечислены как безопасные, а незнакомое существительное рядом с числом по-прежнему повод отказаться.

Проверка на живом сервере: «зарплата 12 млн» → 12 000 000, описание «зарплата» без прилипшего множителя. «бензин 40 литров» уходит модели, а не сохраняется суммой 40.
...
Реализовано 2026-08-14 (TDD). Ветка наличных требовала одновременно глагола «снял» и слова из списка «налич/нал/cash/nakd». Список расширен тем, **откуда** берутся наличные: банкомат, карта, счёт, и их узбекские и латинские написания. Назвать источник — тот же способ сказать «наличные», и именно так люди пишут на самом деле.

Конфликта с переводами не возникло: ветка наличных требует глагола «снял», а перевод — «перевёл», поэтому «перевел на карту» остаётся переводом. Проверено существующими тестами семантики.

Тест сформулирован мягко: фраза считается разобранной верно, если она либо классифицирована как снятие, либо отдана модели. Недопустим только уверенный обычный расход — именно это и было багом.

---

### FT-073: Выданный долг попадал в расходы
...
Context:
Найдено 2026-08-15 при проверке Supabase-репозиториев.

`createDebt` и `payDebt` при создании связанной транзакции задавали только направление
(`type: 'expense' | 'income'`), но не семантику. `normalizeSemanticType` выводила её из
направления, и выданный долг сохранялся как `semanticType: 'expense'`.

Последствие видно на живых данных. Один и тот же долг на 777 777:

| Поверхность | Что показывала |
|---|---|
| `/api/dashboard/:id/quick-stats` | исключала (фильтр `!t.isDebtRelated`) |
| `/api/transactions/analytics/categories/:id` | **23.15% всех расходов**, вторая строка после покупок |

Два эндпоинта давали разные ответы про одни и те же деньги.

Фильтр в разборе по категориям был правильный — он идёт через `countsAsRealExpense`,
как требует И-1. Врали данные: запись сама заявляла, что она обычный расход.

- [x] `createDebt.createLinkedTransaction` ставит `semanticType: 'debt'`
- [x] `payDebt` ставит то же самое
- [x] Тесты в `tests/debt.test.ts` проверяют семантику, а не только направление
- [x] Проверено на работающем сервере: до правки долг в разборе есть, после — нет

Исправлено 2026-08-15 (TDD). Оба теста уже проверяли связанную транзакцию, но спрашивали
только про направление и флаг — семантику никто не проверял, поэтому дыра прожила незамеченной.

Старые записи не тронуты: у них по-прежнему `semanticType: 'expense'`. Это тот самый случай,
ради которого сделан `npm run backfill:preview` — решение о переписывании истории за человеком.

---

### FT-074: Долговая связка не переживала запись в БД
...
Context:
Найдено 2026-08-15 там же.

`analyticsService` в двух местах исключает долговые движения через `isDebtRelated`
(строки 194 и 238). Колонки есть в обеих схемах: TypeORM-сущность объявляет
`isDebtRelated`/`relatedDebtId`, миграция `003_create_debts_table.sql` добавляет
`is_debt_related`/`related_debt_id`. Домен их несёт, `createDebt` их передаёт.

Ни один из двух репозиториев их не писал и не читал. Поле приходило `undefined`,
`!undefined === true` — фильтр не исключал ничего, а ветка на строке 238 не срабатывала
никогда. Комментарий в `createDebt.ts:48` «Transaction is created and linked via
isDebtRelated + relatedDebtId fields» описывал то, чего не происходило.

- [x] `SqliteTransactionRepository` пишет и читает оба поля
- [x] `SupabaseTransactionRepository` пишет и читает `is_debt_related`/`related_debt_id`
- [x] Тесты на обе реализации

Исправлено 2026-08-15 (TDD).

---

### FT-075: SQLite молча терял смену категории
...
Context:
Найдено 2026-08-15 при сравнении двух реализаций репозитория транзакций.

`UpdateTransactionUseCase` передаёт `category` в `repository.update()`. Supabase её писала,
SQLite — нет: в её `update()` просто не было строки для этого поля. Запрос возвращал успех,
категория оставалась прежней.

Хуже того, на этом стоит обучение: `UpdateTransactionWithLearningUseCase` сравнивает запись
до и после, чтобы учиться на исправлениях. Раз категория не менялась, система считала, что
исправления не было, и не училась на самом частом виде правок.

Баг жил только в SQLite — то есть в том, на чём шла вся локальная проверка, и не проявлялся
в проде. Обратная сторона обычного страха «в проде иначе».

- [x] `SqliteTransactionRepository.update` пишет `category`
- [x] Тест на сохранение исправленной категории

Исправлено 2026-08-15 (TDD).

---

### FT-076: Supabase-репозитории без единого теста
...
Context:
Прод работает на Supabase, вся проверка шла на SQLite. Десять файлов реализаций,
`grep -rn "new Supabase" tests/` возвращал ноль. Любое расхождение между двумя
реализациями было невидимо отсюда — что и подтвердилось (FT-074, FT-075).

Сделано:
- [x] `tests/supabaseTransactionRepository.test.ts` — поддельный клиент записывает
      собранный запрос, поэтому тест проверяет, что репозиторий спрашивает у базы,
      без живого подключения
- [x] `tests/sqliteTransactionRepository.test.ts` — то же для SQLite
- [x] Каждый тест проверен мутацией: подмена имени колонки и снятие фильтра архива
      роняют ровно тот тест, который за это отвечает

Мутационная проверка здесь обязательна: тесты написаны на существующий код и прошли
сразу. Тест, который никогда не падал, ничего не доказывает.

Не покрыто: бюджеты, долги, пользователи, подписки, лимиты. Наборы методов у них
сверены и совпадают, мапперы полей сверены поле-в-поле, расхождений не найдено.

---

### FT-072: Low confidence never reaches the review queue
...
Context:
Обнаружено 2026-08-14 при первой проверке голосового ввода.

Голосовой ввод работает и распознаёт семантику верно, но каждая созданная им транзакция получает `confidence` 0.5–0.6 — модель прямо сообщает, что не уверена. При этом `needsReview` остаётся `false`, и запись попадает в итоги неотличимо от точной.

Порог существует, но живёт не там: `messageHandlers.ts:26` задаёт `CONFIDENCE_THRESHOLD = 0.6` и при её недостижении показывает в Telegram клавиатуру подтверждения. Это единственное место, где уверенность на что-то влияет. Транзакция, созданная через API — из Mini App или напрямую, — проходит мимо этой защиты полностью.

Инвариант И-4 требует, чтобы уверенность отражала реальность, «иначе очередь на проверку бесполезна». Формально он соблюдён — система не заявляет `confidence: 1` о том, в чём не уверена. Нарушен его смысл: она честно говорит «уверен наполовину», и никто этого не слышит.

Голосовой ввод страдает сильнее прочих: расшифровка добавляет пунктуацию, из-за чего фраза всегда признаётся сложной и уходит в модель, а та возвращает умеренную уверенность даже на простых фразах.

Definition of Done:
- [ ] Решено, где живёт порог: в use case создания или в каждом канале отдельно
- [ ] Низкая уверенность ставит `needsReview` независимо от канала
- [ ] Telegram-подтверждение и очередь проверки не противоречат друг другу
- [ ] Значение порога выбрано осознанно: 0.6 сейчас отсекает почти весь голосовой ввод
- [ ] Тест: транзакция с уверенностью ниже порога, созданная через API, попадает в очередь
...
- [x] Исчерпание попыток логируется как ошибка
- [x] Тест на восстановление после разрыва
- [ ] Состояние бота в `/api/health` — не сделано, см. ниже

Реализовано 2026-08-14 (TDD): `src/delivery/messaging/telegram/launchWithRecovery.ts`, пять попыток с удвоением паузы от 2 до 30 секунд.

Две вещи выяснились по ходу и определили устройство:

**`launch()` не бросает синхронно.** Telegraf возвращает промис, который отклоняется чуть позже, поэтому наивная проверка объявляла бота работающим за мгновение до того, как узнавала об отказе. Добавлен короткий период, в течение которого запуск должен себя проявить.

**Промис `launch()` разрешается при остановке поллинга, а не при старте.** Отсюда старая строка «Telegram bot started», которая не печаталась никогда. Теперь готовность сообщается в момент начала поллинга, а разрешение промиса трактуется как остановка.

Исчерпание попыток логируется как ошибка с прямой формулировкой: у продукта не осталось основного способа ввода. Раньше это было предупреждение, и снаружи всё выглядело нормально, потому что `/api/health` продолжал отвечать 200.

Существующий тест пришлось переформулировать: он проверял механизм — что на `launch()` навешан `.catch()`. Обработка отказа теперь устроена иначе, но намерение то же, поэтому тест переписан на поведение: запуск не бросает исключение и не оставляет необработанного отклонения.

Пункт про `/api/health` намеренно оставлен открытым: показывать состояние бота в проверке здоровья — решение о контракте эндпоинта, его стоит принимать отдельно, а не походя.
...
Definition of Done:
- [x] «Расходы по категориям» включают только реальные расходы без `needsReview`
- [x] Доходы исключены из диаграммы расходов
- [x] Проценты считаются от суммы реальных расходов
- [x] «Тренды по месяцам» используют тот же фильтр
- [x] Итог аналитики сходится с «реальными расходами» главного экрана до сума
- [x] Регрессионный тест закреплён сторожем инварианта И-1/И-3

Реализовано 2026-08-14 (TDD).

Правка оказалась уже, чем ожидалось: `getSpendingPatterns`, `getTopCategories`, `getMonthlyTrends` и базовая сводка **уже** фильтровали через `countsAsRealExpense`. Не фильтровал ровно один метод — `getDetailedCategoryBreakdown`, тот самый, что питает диаграмму.

Отдельно стоит отметить: два существующих теста в `tests/analytics.test.ts` фиксировали именно неверное поведение — утверждали `breakdown.Income.amount = 200` и процент 47.06, где знаменатель включал доходы. Это был не тест замысла, а слепок реализации, поэтому ожидания исправлены, а не подогнаны.

Проверка на живых данных: разбор по категориям даёт 6 875 246, дашборд на тех же данных — те же 6 875 246. До правки было 25 485 000 против 2 758 000.
...
Definition of Done:
- [x] Выбран и задокументирован единый масштаб
- [x] `getBudgetsNearLimit` приведён к нему
- [x] Публичный контракт эндпоинта не изменился
- [x] Бюджет на 48% не попадает в `nearLimit`, на 85% — попадает
- [x] Тест на границе: 79% / 80% / 81%
- [x] Карточка «Что важно сейчас» согласована с бейджами

Реализовано 2026-08-14 (TDD).

Масштаб выбран в пользу доли: снаружи порог приходит как `?threshold=0.8`, так же его шлёт webapp. Менять публичный контракт ради внутреннего несоответствия было бы дороже, поэтому конвертация происходит внутри сервиса — там, где встречаются две шкалы.

Тесты покрывают границу 79/80/81, явный порог, значение сильно ниже порога и уже превышенный бюджет, который «близким к лимиту» считаться не должен.
...
Решения по открытым вопросам (приняты 2026-08-14):

**Не отдельное поле, а замена существующего.** Форма уже спрашивала «Тип транзакции: расход или доход». Добавить рядом второй вопрос о виде движения значило бы спрашивать почти одно и то же дважды — и позволить сохранить «перевод себе», помеченный доходом, чего ни одна поверхность не смогла бы осмыслить. Поэтому поле одно: «Тип операции» с восемью вариантами, а направление денег выводится из ответа функцией `deriveTransactionType`.

**Показывать всегда.** Вариант «показывать по намёку из категории» отвергнут: он делает поведение формы непредсказуемым и требует угадывать намерение. Восемь пунктов в одном списке дешевле.

**По умолчанию `expense`.** «Определить автоматически» в ручной форме не имеет смысла — определять не по чему, текста нет. Обычный случай стоит первым и не требует ни одного лишнего нажатия.

Definition of Done:
- [x] Решение по вопросам выше зафиксировано здесь
- [x] Семантический тип задаётся при ручном создании
- [x] Значение по умолчанию не удлиняет обычный сценарий
- [x] Созданное вручную не-расходное движение исключается из расходов
- [x] Тест на приём каждого семантического типа обеими формами

Реализовано 2026-08-14 (TDD).

Быстрое добавление разобрано отдельно: восемь вариантов там убили бы смысл «за 10 секунд». Оставлены две кнопки для обычных случаев, третья — «Другое» — раскрывает остальные шесть. Проверено: по умолчанию спец-типов на экране ноль.

Сквозная проверка: «Перевод себе», созданный через форму, сохранился с `semanticType: own_transfer` и направлением `expense`, а расходы на дашборде не изменились — 6 875 246 до и после.

Инвариант И-5 закреплён сторожем `semanticEntry.test.ts`: обе схемы принимают любой семантический тип и отвергают выдуманный.
...
- [x] Прогноз выгорания переформулирован однозначно
- [x] Красный используется только для превышения, не для прогноза
- [x] Скриншот-проверка выполнена

Реализовано 2026-08-14 (TDD).

Дубль списка: страница рисовала виджет `BudgetOverview`, а следом секцию «Все бюджеты» с теми же бюджетами. Виджет принадлежит главному экрану — у него и ссылка «Все →» ведёт сюда же. Со страницы бюджетов он убран, заголовок «Все бюджеты» вместе с ним: когда список один, называть его незачем.

Подпись периода: месяц брался из даты начала, а бюджет, начатый 31 июля, почти целиком приходится на август — отсюда «Месячный • июль 2026» над августовскими тратами. Теперь месяц определяется по середине окна, то есть по тому месяцу, которому принадлежит основная часть трат.

Дубль срока: карточка печатала и «До конца месяца • 17 дней осталось», и отдельной строкой «17 дней осталось». Вторая убрана. Поле `_daysRemainingText` в модели осталось — его читает `BudgetProgress`, дублирование было в карточке, а не в данных.

Прогноз выгорания: «Закончится 17 авг.» красным читалось как срок окончания периода. Переформулировано в «Такими темпами хватит до 17 авг.» — это явно прогноз при текущем темпе. Красный остался только за превышением.

Замер: высота страницы 2254 → 1719px, каждое название бюджета встречается один раз, «июль» в подписях больше не появляется.
...
- [x] Время операции убрано
- [x] Дублирующий бейдж «Расход» убран; не-расходные пометки сохранены
- [x] Проверено на живом списке из 41 транзакции
- [x] Скриншот-проверка выполнена

Реализовано 2026-08-14 (TDD).

Время: строка печатала `createdAt`, то есть момент вставки записи, а не время операции — поэтому все засеянные транзакции показывали одно и то же «00:31». Строки уже сгруппированы заголовками дат, так что время убрано целиком, а не «починено»: показывать точное время операции продукт всё равно не умеет, пользователь его не вводит.

Бейдж: `Расход` стоял в каждой строке и повторял то, что уже сказано цветом и знаком суммы. Введено правило `shouldShowSemanticBadge` — бейдж показывается только для движений, которые не являются ни обычным расходом, ни обычным доходом. Правило намеренно выражено через `isNonExpenseMovement`, и тест это фиксирует, чтобы два списка не разъехались.

Описание: вместо обрезки в одну строку — перенос на две (`line-clamp-2`). Заодно убран тултип для длинных описаний: на телефоне он недоступен, то есть решал проблему только на десктопе.

Замер на живом списке из 41 транзакции: бейджей «Расход» — 0, отметок времени — 0, обрезанных описаний — 0, информативных пометок — 20.
...
- [x] Проверено на живом списке

Реализовано 2026-08-14.

Переключателем стал сам бейдж «Нужно проверить» — он и так стоял в строке и уже сообщал ровно то, что нужно. Отдельная кнопка «показать варианты» была бы третьим элементом там, где хватает двух. К бейджу добавлена стрелка, которая поворачивается при раскрытии, и `aria-expanded`.

Заметность не пострадала: свёрнутая строка по-прежнему несёт предупреждающий бейдж, то есть видно, что операция ждёт решения. Изменилось лишь то, что семь вариантов ответа не занимают место, пока их не спросили.

Сделано в обоих потребителях — `TransactionListItem` и `TransactionCard`. Заодно карточка получила правило показа семантического бейджа из FT-056: раньше она печатала «Расход» так же, как это делала строка.

Проверено на живом списке: по умолчанию чипсов ноль, после нажатия на бейдж появляются, повторное нажатие сворачивает. Раскрытие одной строки добавляет к списку около 156px — столько занимал каждый спорный ряд постоянно.
...
- [x] Аудит-скрипт починен (сделано 2026-08-13 вместе с cloudflared)
- [x] Скрипты проверены на чистом запуске

Реализовано 2026-08-14. Сидер переписан целиком: вместо сырого SQL по схеме января он ходит через API. Так он не может разойтись со схемой, и всё, что он пишет, проходит ту же валидацию, что и ввод живого пользователя.

Данные сделаны детерминированными вместо случайных: по одной операции каждого семантического типа с суммами, которые легко проверить глазом. Это же делает фикстуру пригодной для FT-044 — её итоги можно утверждать.

Две вещи всплыли при первом прогоне и учтены: `--reset` чистил только транзакции, из-за чего второй запуск упирался в лимит долгов 5/5; и засев — это всплеск записей, ровно то, против чего работает ограничитель, поэтому добавлен отступ при 429 с понятным сообщением вместо падения на середине фикстуры.

Замечание про шрифт из старого DoD снято: оно относилось к моей временной оснастке, а не к скрипту проекта.
...
Definition of Done:
- [x] Локальный запуск поднимает API, бот и Mini App одной командой
- [x] Бот работает по polling без вебхука и без публичного хоста
- [x] Mini App открывается через туннель, адрес подставляется автоматически
- [x] Инструкция записана в `quick-start.md`
- [x] Явно указано, какие переменные обязательны и почему

Открытый вопрос закрыт: туннель — **cloudflared**. Выбор был предопределён репозиторием: `scripts/dev-miniapp.js` изначально написан под него. Мешало лишь то, что бинарь искался по жёсткому пути, а адрес писался в `.env` вместо `.env.local` — обе поломки устранены 2026-08-13. Быстрый туннель Cloudflare не показывает страницу-предупреждение, в отличие от бесплатного ngrok, что для Mini App существенно.

Реализовано 2026-08-14: раздел в `quick-start.md`.

Записано главное, что стоило нам получаса разбирательств: адрес туннеля одноразовый, а кнопки в уже отправленных сообщениях Telegram хранят его внутри себя и не обновляются. Поэтому после смены туннеля нужен свежий `/start`, а кнопка меню рядом с полем ввода всегда актуальна, потому что её скрипт правит через API.

Отдельно описан режим без Mini App и объяснено, почему два флага не случайны: `DATABASE_TYPE=sqlite` (в `.env.local` стоит `supabase`, который на паузе) и `ENABLE_TELEGRAM_POLLING=false` (иначе процесс поднимет бота с боевым токеном).

Постоянный адрес остаётся открытым: требует именованного туннеля и домена в Cloudflare, а `sapaev.uz` обслуживается на `ahost.uz`. Решение за Шукуром — переносить DNS или обойтись одноразовым.
...
Definition of Done:
- [x] `кофе 25000` → расход с верной суммой
- [x] `перевел 500000 на Alif` → перевод себе
- [x] `положил на вклад 1000000` → вклад
- [x] `снял в банкомате 300000` → снятие наличных
- [x] Аналитика, бюджеты и дашборд не включают переводы, вклады и долги
- [x] Расхождения оформлены отдельными задачами

Реализовано 2026-08-14. Проверка оставлена как инструмент, а не разовый прогон: `npm run smoke:semantics`.

Она доказывает то, чего не доказывают юнит-тесты: что части согласны между собой на живых данных. Дашборд, разбор по категориям и хранимые строки описывают одни и те же деньги одинаково.

Результат на фикстуре — всё сходится:

| Проверка | Значение |
|---|---|
| реальные расходы | 2 532 000 |
| доходы | 12 000 000 |
| сумма по категориям равна расходам | да |
| ждут решения | 2 |
| не-расходных движений | 6 |

С флагом `--parse` дополнительно проверяется разбор текста через OpenAI: «кофе 25000», «зарплата 12 млн», «перевел 500000 на Alif», «положил на вклад», «снял в банкомате» — все пять распознаны верно.

Расхождений не нашлось: те, что были, закрыты раньше как FT-052, FT-053, FT-067 и FT-068. `/week` отдельным пунктом не проверялся — он ходит в те же функции недельного разбора, что покрыты тестами.
...
- [x] Вывод показывает счётчики и список спорных случаев
- [x] Низкая уверенность не переписывается молча
- [x] Решение о применении остаётся за Шукуром

Реализовано 2026-08-14: `npm run backfill:preview`.

Ключевое решение — не заводить вторую логику классификации. Ветвление по ключевым словам вынесено из парсера в общий `classifyByText`, и предпросмотр ходит в него же. Иначе он предлагал бы типы, которые сам продукт той же формулировке никогда не присвоил бы, и доверять отчёту было бы нельзя.

Отдельно выделен класс, который **нельзя чинить по тексту**: записи, где направление «доход», а семантика «расход». Два поля противоречат друг другу, и никакое чтение описания этого не разрешит — нужен ручной разбор. На локальной базе такая запись нашлась одна, январская «Salary».

На фикстуре скрипт не предлагает ничего: 9 записей имеют осознанно заданный тип, у 15 формулировка не даёт уверенности. Это и есть требуемое консервативное поведение — молчать, когда не уверен.

Настоящая история лежит в Supabase, который на паузе, поэтому запуск против неё — шаг после FT-046.
...
Definition of Done:
- [x] `sqlite3` поднят до 6.0.1
- [x] `npm run verify` зелёный
- [x] SQLite-путь проверен вручную
- [x] `npm audit` чист

Реализовано 2026-08-14. Апгрейд снял всю оставшуюся цепочку `sqlite3 → node-gyp → tar/cacache`: **уязвимостей ноль**, включая ту critical в `tar`.

Ручная проверка на живом сервере важнее зелёного `verify`: тесты ходят в моки репозиториев и настоящий драйвер не трогают. Прогнан полный цикл — создание, чтение, обновление, архивация, возврат из архива, удаление. Все шаги 200.
...
Definition of Done:
- [x] Каждое открытое Issue помечено: сделано / актуально / переосмыслено
- [ ] Закрытие Issues на GitHub — требует решения Шукура
- [ ] Источник правды зафиксирован — требует решения Шукура
- [ ] `CLAUDE.md` приведён в соответствие

Разбор выполнен 2026-08-14. Сами Issues не тронуты: закрывать их — действие в публичном репозитории, и оно за владельцем.

**Сделано, можно закрывать (4):**

| Issue | Чем перекрыто |
|---|---|
| #74 quick-add на главной | Сделано ещё в FT-027C/FT-031C; на главной есть быстрое добавление |
| #78 «7 UX проблем» | Перекрыто редизайном FT-027…FT-037 и вехой 2 (FT-055…FT-058) |
| #79 Drawer для QuickAdd | Шторки уже используются, включая новую «Ещё» |
| #80 shadcn Tabs | Заменено собственными сегментированными вкладками (FT-029) |

**Сделано частично (1):**

| Issue | Что осталось |
|---|---|
| #73 weekly insights в боте | Команда `/week` работает. Незакрытой осталась только рассылка по расписанию — issue стоит сузить до неё |

**Актуальные (9):** #70 (базовые классы репозиториев), #81 (Analytics v2), #71 (повторяющиеся операции), #72 (экспорт), #75 (мультивалютность), #76 (цели накопления), #77 (онбординг), #83 (публичный API), #84 (ShareCost).

Из них #81 после FT-052 стоит переформулировать: данные аналитики теперь считаются верно, вопрос сместился с «починить» на «что ещё показать».

**Про источник правды.** Сейчас их два, и они не пересекаются: `TASKS.md` содержит 105 задач по качеству и корректности, GitHub Issues — 14 продуктовых замыслов. Фактически это не дубль, а разделение по природе: доска про «как чинить», Issues про «что построить». Возможно, честнее закрепить это разделение, чем сводить всё в одно место. Решение за Шукуром.
...
Definition of Done:
- [x] Для каждой ветки определено, влита она или содержит незабранные изменения
- [x] Невлитые изменения описаны здесь
- [x] Удалены только полностью влитые ветки
- [x] Ни одного force push
- [ ] Судьба семи невлитых веток — за Шукуром

Сделано 2026-08-14. Удалено восемь веток, влитых в `main` целиком. Использован `git branch -d`, который сам отказывается удалять невлитое, — то есть потерять что-либо этой операцией было невозможно.

**Осталось семь веток с незабранными коммитами.** Удалять их я не стал: это единственное место, где работа исчезнет безвозвратно.

| Ветка | Своих коммитов | Последний | Что внутри |
|---|---|---|---|
| `feat/new-frontend` | 31 | 2026-01-15 | Перенос `webapp-v2` в `webapp`, навигация, раскладка. Похоже, влито вручную — текущий фронт на этом и стоит |
| `fix/tests` | 13 | 2026-01-15 | Моки репозиториев, удаление старой `design-system/` |
| `by-codex` | 11 | 2025-06-26 | Сервис аналитики с cron, ассистент на SSE — функциональность, которой в `main` нет |
| `codex/создать-фронтенд-для-mini-app-в-телеграм` | 2 | 2025-07-03 | Скелет React-приложения |
| `refactor/by-gpt` | 2 | 2025-05-27 | Внедрение зависимостей, чистка каталогов |
| `codex/проверить-архитектурные-проблемы...` | 1 | 2025-06-09 | Единая настройка модулей, интерфейс транскрипции |
| `fix/dockerfile` | 1 | 2025-07-15 | Копирование каталога downloads в образ |

Три верхние — январские и, судя по состоянию `main`, их содержимое так или иначе доехало. Четыре нижние — прошлогодние, им больше года.

Отдельно стоит посмотреть `by-codex`: одиннадцать коммитов с аналитикой по расписанию и ассистентом на SSE. Такого в `main` нет, и это единственная ветка, где может лежать что-то нужное.
