...
## Active Plan — Path to Relaunch (2026-08-12)

Цель отрезка: довести проект до состояния «одной кнопкой поднял и пользуюсь сам».

Контекст на момент планирования:

- Прод-сервер `sapaev.uz` / `56.228.14.14` выключен намеренно. Порты 22 и 443 не отвечают.
- Supabase-проект «Finance Tracker» (AWS `ap-south-1`) **на паузе, не удалён** — база и история целы, поднимается кнопкой Resume.
- CD (`deploy.yml`) падает на каждом пуше с `dial tcp ***:22: i/o timeout`. Последний успешный деплой — **2026-01-27**. Вся работа с февраля по июль в прод не попадала.
- Миграции `007_add_transaction_semantic_type.sql` и `008_add_transaction_needs_review.sql` к Supabase не применялись.
...
| Веха | Смысл | Задачи |
|------|-------|--------|
| 0. Разведка | Снять последнюю неизвестность | FT-042 |
| 1. «Могу пользоваться» | Рабочий локальный контур с реальным вводом | FT-043, FT-044, FT-045 |
| 2. «Готов к включению» | Прод поднимается без сюрпризов | FT-046, FT-047, FT-048 |
| 3. Порядок в трекинге | Один источник правды по задачам | FT-049, FT-050 |
| 4. Продукт P1 | Скорость ввода и система внимания | FT-051 |

Порядок вех менять можно, но веха 2 не начинается раньше, чем веха 1 подтвердила, что семантика работает на живом вводе.
...
Context:
Прод выключен намеренно, но неизвестно, остановлен инстанс или терминирован. От этого зависит, чинится ли CD правкой secrets или нужен новый хост целиком. Supabase уже прояснён — проект на паузе и восстановим.

Definition of Done:
- [ ] Определить состояние инстанса `56.228.14.14`: stopped / terminated / отсутствует
- [ ] Если stopped — зафиксировать, сохранится ли публичный IP после запуска (Elastic IP или нет)
- [ ] Проверить, совпадает ли `SSH_HOST` в GitHub Actions secrets с реальным адресом
- [ ] Сверить ID Supabase-проекта в `CLAUDE.md` (`cttsquvkvkwtxsfrgsrs`) с тем, что виден в дашборде
- [ ] Записать результат в этот файл

Notes:
- MCP-доступ к Supabase из текущей сессии видит только проект `Tezzy` (`rnmxtzyqcrtpoxmabniv`), а на финтрекерный отдаёт `You do not have permission`. Возможно, MCP подключён к другому аккаунту — стоит проверить заодно.
...
Blocked by:
- FT-042 (доступ и подтверждение аккаунта)
- Явное разрешение Шукура: применение SQL к проду имеет внешний эффект

---

### FT-047: Repair production deploy pipeline
...
Context:
`deploy.yml` падает 83 раза подряд на `dial tcp ***:22: i/o timeout`. Причина — выключенный хост, но после его запуска могут не совпасть IP и ключи. Пайплайн, который годами красный, перестаёт быть сигналом.

Definition of Done:
- [ ] Инстанс поднят и доступен по SSH
- [ ] Secrets (`SSH_HOST`, ключ, пользователь) сверены с реальным хостом
- [ ] Первый зелёный прогон `deploy.yml` с 2026-01-27
- [ ] `/api` и Mini App отвечают на публичном домене
- [ ] Решено, что делать с пушами при выключенном сервере: пропускать деплой или помечать skipped, чтобы красный статус снова что-то значил
