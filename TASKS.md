...
Definition of Done:
- [ ] Транзакция с пустым «Местом» сохраняется
- [ ] `null` и `undefined` для необязательных полей обрабатываются одинаково
- [ ] Проверены остальные необязательные поля форм на ту же ошибку (`null` из API против `.optional()`)
- [ ] Сообщения валидации на русском; `Invalid input` в интерфейсе не появляется
- [ ] Тест: загрузить транзакцию с `merchant = null`, изменить сумму, сохранить
...
Definition of Done:
- [ ] Разархивация архивной транзакции возвращает 200 и снимает флаг
- [ ] Проверка владельца для разархивации видит архивные записи
- [ ] Чужую архивную транзакцию по-прежнему нельзя тронуть
- [ ] Проверено на обоих репозиториях
- [ ] Тест на полный цикл: создать → архивировать → вернуть → запись снова в основном списке
