...
## Active Plan — Path to Relaunch (2026-08-12, ревизия 2026-08-24)
...
Ревизия Hermes 2026-08-16: подтянут свежий `origin/main`; активный план 2026-08-12/13 уже новее старого локального backlog. Старые предложения Hermes `FT-042..FT-047` из июльского контекста не возвращать в прежнем виде: AWS/prod-срез отменён или переопределён (например, FT-047 уже переопределена как консервация deploy job и закрыта), subscription expiry остаётся отложенным до решения о живом проде.

Docs-сверка 2026-08-16 (только Markdown, очередь задач не тронута): документация приведена в соответствие с фактическим состоянием репозитория. Исправлено: «8 модулей / DashboardModule» → 7 модулей приложения + `DashboardService`/`DashboardController`, собираемые в Express из Transaction analytics и Budget service; маршруты вебапки приведены к `webapp/src/app/router/routes.tsx` (убраны несуществующие `/dashboard` и `/stats`); статус деплоя описан как есть (`quality-gate` на каждый пуш, `deploy` только вручную); quick-start переписан под `npm run dev:miniapp -- --chat-id=<id>`; описание `npm run verify` дополнено `test:webapp`. На `PROJECT_DOCUMENTATION.md`, `AUDIT.md`, `SUPABASE_MIGRATION.md`, `DEPLOYMENT.md` добавлены баннеры «историческое / не активный путь» со ссылками на `TASKS.md` и `CLAUDE.md`. Порядок реализации FT-задач не менялся.
...
Ревизия 2026-09-04 после FT-076: документационный гайд по iPhone Action Button закрыт — `docs/IOS_SHORTCUT_ACTION_BUTTON.md` описывает безопасный MVP через Telegram (Shortcut спрашивает текст → копирует → открывает чат бота → пользователь отправляет), без нового API, без токенов и без изменений кода. Это исчерпывает безопасную часть quick-capture: всё остальное в этом направлении упирается в `FT-075` (auth/токен/отзыв/rate limit), который остаётся blocked до решения Шукура. Активная очередь снова **FT-059 → FT-060 → FT-061**.
...
Definition of Done:
- [x] Быстрый парсер либо понимает множители (млн, миллион, тыс, тысяч, к, тыщ), либо считает их признаком сложного текста и отдаёт фразу OpenAI
- [x] Ни при каком исходе текст с множителем не сохраняется с потерянным порядком величины
- [x] Множитель не остаётся в описании и merchant
- [x] Тесты: «12 млн», «12 миллионов», «3.5 млн», «25 тыс», «25 тысяч», «15к», «50 тыщ», «2 млн сум»
- [x] Проверить обратное: «1000000» без слова по-прежнему разбирается верно
...
Definition of Done:
- [x] «снял в банкомате», «снял с карты», «обналичил» распознаются как `cash_withdrawal`
- [x] Проверены узбекские формулировки из существующих паттернов
- [x] Неоднозначные случаи уходят в OpenAI, а не молча в `expense`
- [x] Тесты на каждую формулировку
...
Definition of Done:
- [x] Выбран и задокументирован единый масштаб: доля 0–1 либо проценты 0–100
- [x] `getBudgetsNearLimit` и все вызывающие стороны приведены к нему
- [x] Публичный контракт эндпоинта `alerts?threshold=` не меняет смысл молча
- [x] Бюджет на 48% не попадает в `nearLimit`, на 85% — попадает
- [x] Тест на границе: 79% / 80% / 81%
- [x] Карточка «Что важно сейчас» согласована с бейджами на карточках бюджетов
...
Definition of Done:
- [x] На странице бюджетов каждый бюджет показан один раз
- [x] Период подписан месяцем, к которому относятся траты
- [x] «Осталось N дней» встречается в карточке один раз
- [x] Прогноз выгорания либо переформулирован однозначно, либо убран
- [x] Красный используется только для превышения, не для прогноза
- [x] Скриншот-проверка на 375 / 390 / 412

Verification 2026-09-03:
- `/budgets` now renders aggregate `Итого по бюджетам` once, then per-budget cards once in `Все бюджеты`; the old `BudgetOverview` list duplication is not used on this page.
- Monthly period labels use the spending month plus exact range, e.g. `Сентябрь 2026 • 01.09–01.10`; weekly labels keep the exact range.
- Budget cards show a single days-left line such as `Ещё 3 дня до 6 сентября`.
- Forecast wording uses `Прогноз:` / `Риск:` and red is reserved for actual exceeded budgets; near-limit/risk uses warning.
- Webapp tests: `npm run test:webapp` — 8 files / 45 tests passed.
- Full gate: `npm run verify` — 34 Jest suites / 424 tests, 8 webapp files / 45 tests, backend build, webapp build, dependency-cruiser, madge passed.
- Screenshot QA: `/tmp/ft055-budgets-audit-content/screenshots/budgets-375.png`, `budgets-390.png`, `budgets-412.png`, plus `budgets-390-bottom.png`; authenticated metrics had `issueCount=0` and centered nav button at 187.5/195/206px respectively.
...
Definition of Done:
- [x] Чипсы раскрываются по действию, а не всегда
- [x] Свёрнутое состояние сохраняет заметность: видно, что операция требует решения
- [x] Высота такой строки сопоставима с обычной
- [x] Проверено на списке минимум с пятью `needsReview`

Verification 2026-09-04:
- `TransactionCorrectionChips` now starts collapsed with a compact `Исправить тип` action; semantic correction choices render only after expansion.
- `TransactionListItem` keeps `Нужно проверить` visible and places the correction action next to it, so collapsed rows no longer render the seven chips inline.
- Added pure display helper tests for collapsed/expanded correction labels.
- Webapp tests: `npm run test:webapp` — 9 files / 61 tests passed.
- Full gate: `npm run verify` — 34 Jest suites / 424 tests, 9 webapp files / 61 tests, backend build, webapp build, dependency-cruiser, madge passed.
- Screenshot QA with five disposable `needsReview` rows: `/tmp/ft057-corrections-audit-content/screenshots/transactions-375-collapsed.png`, `transactions-390-collapsed.png`, `transactions-412-collapsed.png`, plus expanded-first screenshots. Authenticated audit `issueCount=0`; five collapsed actions visible; first expanded row shows choices while other rows stay collapsed. Disposable rows deleted with HTTP 200; leak check `0`.
...
Verification 2026-09-04:
- Raised shared small buttons/tabs and known compact controls to 44px minimum hit areas without adding new visual hierarchy.
...
Status: done (dev/test only — **не продовая auth-модель**)
Priority: medium
Owner: Claude Code, QA by Hermes
Type: quick-capture-api

Context:
Прямой вызов capture-эндпоинта из iPhone Shortcut минует Telegram-авторизацию Mini App. Для локального контура (WSL + SQLite + Cloudflare quick tunnel) это решено узко: Shortcut идентифицирует себя заголовком, обход жёстко выключен в production, а вопросы «как выдавать/отзывать персональный токен и как считать лимиты для не-Telegram источника» вынесены в будущее продовое решение и больше не блокируют локальное использование.

Definition of Done:
- [x] `X-Shortcut-User-Id` задаёт владельца записи: значение заголовка перетирает `userId` в теле и принудительно ставит `source=ios_shortcut`
- [x] Опциональный общий токен: если задан `SHORTCUT_CAPTURE_TOKEN`, запрос обязан прислать совпадающий `X-Shortcut-Capture-Token`, иначе 401 `INVALID_SHORTCUT_TOKEN`
- [x] `NODE_ENV=production` → 403 `SHORTCUT_CAPTURE_DISABLED` (шумный отказ, а не тихий фолбэк на другой auth-путь)
- [x] Запросы без заголовка идут прежним путём через `allowGuestMode` — Mini App и guest не затронуты
- [x] `req.telegramUser` не подделывается, поэтому `verifyOwnership`/`requireAdmin` остаются fail-closed для shortcut-запросов
- [x] `.env.example` содержит пустой плейсхолдер `SHORTCUT_CAPTURE_TOKEN=` с пометкой DEV/TEST ONLY
- [x] Контракт и границы описаны в `docs/QUICK_CAPTURE_API.md` и `docs/IOS_SHORTCUT_ACTION_BUTTON.md`
- [x] Тесты на все ветки (заголовок/без заголовка, токен задан/не задан/неверный, production)

Реализовано 2026-09-04:

- `allowShortcutCapture` в `src/delivery/web/express/middleware/authMiddleware.ts`; `createQuickCaptureRouter` использует его вместо `allowGuestMode` (`src/modules/quickCapture/presentation/controllers/quickCaptureController.ts`).
- Сравнение токена — constant-time через sha256 + `timingSafeEqual`, чтобы не течь длиной ожидаемого значения.
- Rate limit не менялся: capture по-прежнему под общим с `/api/voice/*` `aiRateLimiter` (per-IP).

Проверка:
- `npm test -- quickCaptureRoutes --runInBand` — 18 тестов passed.
- `npm run verify` — 34 Jest suites / 433 теста, 11 webapp Vitest файлов / 83 теста, backend build, webapp build, dependency-cruiser, madge passed.

Осталось как будущее решение Шукура (не блокер):
- Продовая модель auth: персональный токен вместо общего, срок жизни, отзыв, поведение при утечке.
- Отдельный rate limit для не-Telegram источника.
- Нужно ли вообще включать этот путь в проде или Telegram-канала (`FT-076`) достаточно.

До такого решения: не включать обход в production, не выдавать токен третьим лицам, не класть реальные значения в клиентские инструкции.

---

### FT-076: iPhone Action Button guide via Telegram (docs only)
...
Context:
Безопасная часть iPhone-капчи не требует ни нового эндпоинта, ни токенов: Shortcut может спросить текст, положить его в буфер и открыть чат с ботом, а дальше работает уже существующий Telegram quick capture. Нужен пошаговый гайд для Шукура, который честно отделяет проверяемое по коду от того, что можно проверить только на устройстве, и явно фиксирует, почему прямой вызов API на момент FT-076 был запрещён (`FT-075` тогда был blocked); позже `FT-075` добавил прямой вызов для dev/test-контура, а продовая auth-модель остаётся будущим решением.

Definition of Done:
- [x] `docs/IOS_SHORTCUT_ACTION_BUTTON.md` описывает цель и безопасный MVP через Telegram
- [x] Пошаговая настройка Shortcut на русском, только с плейсхолдером `<BOT_USERNAME>`, без токенов и секретов
- [x] Разделено: что заполняет Шукур и что нельзя проверить из WSL
- [x] Альтернатива без Shortcut (голосовое/диктовка в чате бота, виджет Telegram)
- [x] Прямой `POST /api/quick-capture` из Shortcut на момент FT-076 явно помечен как заблокированный до решения по auth/токену/отзыву/rate limit (позже `FT-075` разблокировал его только для dev/test; продовая auth — будущее решение)
- [x] Чек-лист проверки на реальном iPhone
- [x] Troubleshooting и ограничения
- [x] Изменения только в документации, без кода и без коммита

Реализовано 2026-09-04 (docs-only):

- Основной вариант A — «запросить ввод → скопировать в буфер → открыть `tg://resolve?domain=<BOT_USERNAME>`», плюс вариант B (лист «Поделиться») и вариант C (префилл через `&text=`) как непроверяемый из WSL эксперимент.
- Зафиксировано по коду, а не предположением: `?start=<payload>` не годится, потому что приходит как команда, а обработчик записи пропускает всё, что начинается с `/` (`src/delivery/messaging/telegram/handlers/messageHandlers.ts:135`).
- Ограничения выведены из фактического контракта: нет `occurredAt`, нет идемпотентности, per-IP rate limit общий с `/api/voice/*`, дата операции = момент обработки (важно, когда локальный бот был выключен).
- Явный запрет на перенос в Shortcut `TG_BOT_API_KEY`, `OPENAI_API_KEY`, `X-Dev-User-Id` и Mini App `initData`.

Проверка:
- `grep` по файлу на токен-подобные паттерны (`sk-`, `bot<digits>:`, `Bearer <value>`, `TOKEN=`, `KEY=`) — совпадений с реальными значениями нет, только плейсхолдеры и имена переменных в запрещающем контексте.
...
Context:
Расхождения между `docs/knowledge-base/10-design-guidelines/` и реализацией:
...
Definition of Done:
- [x] Seed-скрипт работает против текущей схемы
- [x] Аудит-скрипт реально авторизуется, на снимках видны данные
- [x] Аудит падает или громко предупреждает, если получил 401 хотя бы на один запрос
- [x] `extraHTTPHeaders` не навешивается на кросс-доменные запросы: иначе ломается загрузка шрифта с Google Fonts и снимки уходят на фолбэк-шрифте
- [x] Оба скрипта проверены на чистом запуске
