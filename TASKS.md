...
Context:
Переформулирована 2026-08-13. Исходно задача была «починить деплой», но AWS-сервер выключен из планов, чинить нечего. При этом `deploy.yml` продолжает запускаться на каждый пуш и падать на `dial tcp ***:22: i/o timeout` — счёт идёт на 84 подряд с 2026-01-27. Каждый пуш присылает письмо о провале, а красный значок перестал что-либо означать.

Definition of Done:
- [ ] `deploy.yml` перестаёт запускаться автоматически: отключён либо переведён на ручной запуск
- [ ] В файле рабочего процесса записано, почему он остановлен и что нужно для возврата
- [ ] Пуш в `main` больше не порождает падающий прогон
- [ ] Остальные workflow (`claude.yml`, `claude-code-review.yml`, `docker-cleanup.yml`) проверены: не завязаны ли они на тот же хост

Notes:
- Не удалять файл: при возврате к хостингу переписывать с нуля дороже, чем расконсервировать.
